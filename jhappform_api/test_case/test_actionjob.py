...
class TestActionjob(unittest.TestCase):
    """测试 appform 操作单个作业 case："""

    def setUp(self):
        print("开始测试操作（kill、peek、resume、stop、top、bot、hist）单个作业【action api】 ...")


    def actions(self, arg1,arg2,arg3,arg4):
        self.url = arg4[0] + "job/" + arg2 + "/"  + arg1 + "?token=" + arg4[1]
...
def generateTestCases(cases):
    ...
    for args in arglists:
        setattr(TestActionjob, 'test_actionjob_{0}_{1}_{2}{2}_{4}'.format(args[0], args[1], args[2], args[3], args[4]), TestActionjob.getTestFunc(*args) )


generateTestCases(main.get_test_data(type='actionjob'))
