...
class TestDelspooler(unittest.TestCase):
    """测试 appform 立即删除数据目录 case："""

    def setUp(self):
        print("开始测试立即删除数据目录【del api】 ...")

    def tearDown(self):
        print("【del api】 访问的URL地址为：")
        print(self.url)
        print("【del api】 测试数据为：")
        print(self.data)
        print("【del api】 测试返回值：")
    ...
    def actions(self, arg1,arg2,arg3):
        self.url = arg3[0] + "/spooler/del/" + arg1 + "?token=" + arg3[1]
...
def generateTestCases(cases):
    ...
    for args in arglists:
        setattr(TestDelspooler, 'test_delspooler_{0}_{1}{1}_{3}'.format(args[0], args[1], args[2], args[3]), TestDelspooler.getTestFunc(*args) )


generateTestCases(main.get_test_data(type='delspooler'))
