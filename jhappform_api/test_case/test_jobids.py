...
class TestJobids(unittest.TestCase):
    """测试 appform 根据作业 id 列表查询作业 case："""

    def setUp(self):
        print("开始测试根据作业 id 列表查询作业【jobsbyid api】 ...")

    def tearDown(self):
        print("【jobsbyid api】 访问的URL地址为：")
        print(self.url)
        print("【jobsbyid api】 测试数据为：")
        print(self.data)
        print("【jobsbyid api】 测试返回值：")
        print(self.result)
        print("【jobsbyid api】 测试结束...")


    def actions(self, arg1,arg2,arg3):
        self.url = arg3[0] + "jobsbyid?id=" + arg1 + "&token=" + arg3[1]
...
def generateTestCases(cases):
    ...
    for args in arglists:
        setattr(TestJobids, 'test_jobids_{0}_{1}{1}_{3}'.format(args[0], args[1], args[2], args[3]), TestJobids.getTestFunc(*args) )


generateTestCases(main.get_test_data(type='jobids'))
